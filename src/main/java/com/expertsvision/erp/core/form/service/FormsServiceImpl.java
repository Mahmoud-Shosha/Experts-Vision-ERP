...
public class FormsServiceImpl implements FormsService {
	...
	public void updateFormsView(UsersView loginUser, FormsView formsView) {
		...
		if (!generalDAO.isEntityExist("forms", conditions)) throw new ValidationException("not_exist", "form_no");
		conditions.clear();
		conditions.put("module_no", form.getModuleNo());
		if (!generalDAO.isEntityExist("modules", conditions)) throw new ValidationException("not_exist", "module_no");
		conditions.clear();
		conditions.put("form_no", form.getParentForm());
		if (!generalDAO.isEntityExist("forms", conditions)) throw new ValidationException("not_exist", "parent_form");
		conditions.clear();
		conditions.put("form_d_name", form.getFormDName());
		String exceptionCondition = null;
		exceptionCondition = " and form_no != " + form.getFormNo();
		if (generalDAO.isEntityExist("forms", conditions, exceptionCondition)) throw new ValidationException("already_exist", "name");
		if (form.getFormFName() != null) {
			conditions.clear();
			conditions.put("form_f_name", form.getFormFName());
			if (generalDAO.isEntityExist("forms", conditions, exceptionCondition)) throw new ValidationException("already_exist", "foreign_name");
		}
		// Update the from
